...
exports.respondToNewGame = async ({ actions, user, channel, message_ts }) => {
  ...
  if (message_ts !== currentGame.messageTs) {
    // Somehow user clicked a button from an old game, so send a response
    // only to the user about that
    bot.chat.postEphemeral(
      channel.id,
      `Hmmm, that game doesn't exist anymore. Did you accidentally click an old button? Check <#${global.FOOSBOT_CHANNEL_ID}|${process.env.FOOSBOT_CHANNEL_NAME}> to see if there is a new game being organized!`,
      user.id
    )
  }
  ...
  if (currentGame.ins.size === 3) {
    ...
    // If this user will be the 4th, the game is ready. Update the message
